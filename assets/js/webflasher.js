...
document.addEventListener("DOMContentLoaded", () => {
    const manifestRadios = Array.from(document.querySelectorAll("[data-manifest-option]"));
    const channelRadios = Array.from(document.querySelectorAll("[data-release-channel]"));
    ...
    let currentChannel = channelRadios.find((radio) => radio.checked)?.value ?? channelRadios[0]?.value;
    const applyManifest = (radio) => {
        ...
        const channelLabel = channelRadios.find((radio) => radio.value === currentChannel)?.dataset.label ?? currentChannel;
